...
/**
 * Expose `Mixtore` integration.
 */

var Mixtore = exports = module.exports = integration('Mixtore')
  .option('apiKey', '')
  .option('apiHost', 'analytics.uservice.mixtore.com')
...
Mixtore.prototype.initialize = function() {
  var self = this;
...
Mixtore.prototype.loaded = function() {
  return true;
};
...
Mixtore.prototype.onpage = function(page) {
...
Mixtore.prototype.onidentify = function(identify) {
...
Mixtore.prototype.ongroup = function(group) {
...
Mixtore.prototype.ontrack = function(track) {
  var json = track.json();
...
Mixtore.prototype.onalias = function(alias) {
  var json = alias.json();
...
Mixtore.prototype.normalize = function(msg) {
  ...
  ctx.screen = {
      viewport: w.screen.availWidth + 'x'+ w.screen.availHeight,
      resolution: w.screen.width + 'x' + w.screen.height,
      colors: w.screen.colorDepth

  };
  ctx.language = w.navigator.language;
  ctx.encoding = d.charset;
  msg.partner_id = w.partner_id;
...
Mixtore.prototype.ampId = function(ctx) {
...
Mixtore.prototype.enqueue = function(path, msg, fn) {
  // XXX:  use http or https

  var url = 'http://' + this.options.apiHost + path;
...
Mixtore.prototype.cookie = function(name, val) {
  var store = Mixtore.storage();
...
Mixtore.prototype.referrerId = function(query, ctx) {
 ...
 */
Mixtore.prototype.retrieveCrossDomainId = function(callback) {
